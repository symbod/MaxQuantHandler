...
def get_filter_ids_logging(original, filtered, handler, organism):
    ...
    log_df["Removed IDs"] = log_df.apply(
        lambda row: list(set(row["IDs"]).symmetric_difference(set(row["Filtered IDs"]))), axis=1)
    log_df["Nr IDs"] = log_df["IDs"].str.len().mask(log_df["IDs"].eq(""), 0)
    log_df["Nr Filtered IDs"] = log_df["Filtered IDs"].str.len().mask(log_df["Filtered IDs"].eq(""), 0)
    ...
    if sum(log_df["Nr Removed IDs"]) == 0:
        ...
        df, missing = handler.get_preloaded(in_list=removed_ids, in_type="protein",
                                            organism=organism)  # missing should be empty (because has already been added in the steps before)
        return {"Overview_Log": log_df, "Detailed_Log": df}


# === Logging DataFrame For Remapped Gene Names ===
def get_remapped_genenames_logging(original, remapped):
    return {"Overview_Log": pd.DataFrame(), "Detailed_Log": pd.DataFrame()}

# === Logging DataFrame For Remapped Gene Names ===
def get_ortholog_genenames_logging(original, orthologs):
    return {"Overview_Log": pd.DataFrame(), "Detailed_Log": pd.DataFrame()}


# === Logging DataFrame For Reducing Gene Names ===
def get_reduced_genenames_logging(original, reduced):
    log_df = pd.DataFrame({"Gene Names": original.str.split(";"),
                           "Reduced Gene Names": reduced.str.split(";")})  # easy to compute the next steps
    log_df["Removed Gene Names"] = log_df.apply(
        lambda row: list(set(row["Gene Names"]).symmetric_difference(set(row["Reduced Gene Names"]))), axis=1)
    log_df["Nr Gene Names"] = log_df["Gene Names"].str.len().mask(log_df["Gene Names"].eq(""), 0)
    log_df["Nr Reduced Gene Names"] = log_df["Reduced Gene Names"].str.len().mask(log_df["Reduced Gene Names"].eq(""),
                                                                                  0)
